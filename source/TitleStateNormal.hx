...
class TitleStateNormal extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		logoBl.animation.addByPrefix('bump', 'troublepoopin', 24);
		...
		snowfort = new FlxSprite(-80).loadGraphic('assets/images/friendshipclubimage.png');
		snowfort.scrollFactor.x = 0;
		snowfort.scrollFactor.y = 0;
		snowfort.updateHitbox();
		snowfort.screenCenter();
		snowfort.antialiasing = true;
		add(snowfort);
	...
	override function beatHit()
	{
		...
		{
			case 1:
				snowfort.visible = true;
			case 3:
				addMoreText('present');
				snowfort.visible = false;
			case 5:
				deleteCoolText();
