...
//import openfl.effects.*;
//import openfl.effects.WiggleEffect;
...
class PlayState extends MusicBeatState
{
	...
	public static var computerSongs:Array<String>;
	...
	var torrentBack:FlxTypedGroup<FlxSprite>;
	...
	override public function create()
	{
		...
		computerSongs = ["torrent", "sensory-overload", "jailbreak", "bitcrush", "firewall", "subzero"];
		...
		if (stageCheck == 'computer')
			{
				defaultCamZoom = 0.8;
				curStage = 'computer';

				var monitor:FlxSprite = new FlxSprite(-292, -68).loadGraphic(Paths.image("monitorBack0"));
				// // bg.setGraphicSize(Std.int(bg.width * 2.5));
				// // bg.updateHitbox();
				monitor.setGraphicSize(Std.int(monitor.width * 1.28));
				monitor.antialiasing = true;
				monitor.scrollFactor.set(0.3, 0.3);
				monitor.active = false;
				add(monitor);
				torrentBack = new FlxTypedGroup<FlxSprite>();
				add(torrentBack);

			if (SONG.song.toLowerCase() == "jailbreak"){
				for (i in 4...8)
					{
						var monitorColor:FlxSprite = new FlxSprite(-592, -250).loadGraphic(Paths.image('monitorBack' + i));
						if (i == 4){
							monitorColor.visible = true;
						}else {
							monitorColor.visible = false;
						}
						
						monitorColor.setGraphicSize(Std.int(monitorColor.width * 1.28));
						monitorColor.scrollFactor.set(0.3, 0.3);
						//monitorColor.active = false;
						monitorColor.updateHitbox();
						monitorColor.antialiasing = true;
						torrentBack.add(monitorColor);
					}
				 } else if  (SONG.song.toLowerCase() == "bitcrush"){
					for (i in 9...12)
					{
						var monitorColor:FlxSprite = new FlxSprite(-592, -250).loadGraphic(Paths.image('monitorBack' + i));
						if (i == 9){
				...
				nonbiney = new FlxSprite();
				nonbiney.frames = Paths.getSparrowAtlas("binary");
				nonbiney.animation.addByPrefix('binaryglowin', "binaryglow", 24, true);
				nonbiney.updateHitbox();
				nonbiney.antialiasing = true;
				nonbiney.screenCenter();
				nonbiney.scrollFactor.set(0.5, 0.5);
				nonbiney.setGraphicSize(Std.int(nonbiney.width * 1.65));
				add(nonbiney);

				FlxTween.tween(nonbiney, {y: nonbiney.y - 60}, 2, { type: FlxTween.PINGPONG, ease: FlxEase.sineInOut});
				
				// wiggle effect dont work
				/*
				...
				nonbiney.shader = wiggleEffect.shader;
				*/

				error = new FlxSprite(-320, 46).loadGraphic(Paths.image("riptorrent"));
				error.updateHitbox();
				error.screenCenter();
				error.setGraphicSize(Std.int(error.width * 2));
				...
				var stone:FlxSprite = new FlxSprite(-690, -180).loadGraphic(Paths.image("stageStone"));
				stone.updateHitbox();
				stone.antialiasing = true;
				stone.scrollFactor.set(1, 1);
				stone.active = false;
				add(stone);
		...
		else if (stageCheck == 'computerpixel')
		{
			curStage = 'computerpixel';

			// defaultCamZoom = 0.9;

			var monitorpixel = new FlxSprite().loadGraphic(Paths.image('backgroundpixel'));
			monitorpixel.scrollFactor.set(0.3, 0.3);
			add(monitorpixel);
			monitorpixel.x -= 150;
			monitorpixel.y -= 170;
			...
			var binarypixel:FlxSprite = new FlxSprite(repositionShit, 0).loadGraphic(Paths.image('binarypixel'));
			binarypixel.scrollFactor.set(0.5, 0.5);
			add(binarypixel);
			binarypixel.x -= 430;
			FlxTween.tween(binarypixel, {y: binarypixel.y - 60}, 2, { type: FlxTween.PINGPONG, ease: FlxEase.sineInOut});

			var stonepixel:FlxSprite = new FlxSprite(repositionShit).loadGraphic(Paths.image('platformpixel'));
			stonepixel.scrollFactor.set(1, 1);
			add(stonepixel);

			var widShit = Std.int(monitorpixel.width * 6);
			var widShitTwo = Std.int(binarypixel.width * 8);

			monitorpixel.setGraphicSize(widShit);
			binarypixel.setGraphicSize(widShitTwo);
			stonepixel.setGraphicSize(widShit);

			monitorpixel.updateHitbox();
			binarypixel.updateHitbox();
			stonepixel.updateHitbox();
		}
		...
		if (dad.curCharacter == 'torrent')
		{
			healthBar.createFilledBar(0xFF4876ff, 0xFF66FF33);
		}
		else{
		...
		var vignette:FlxSprite = new FlxSprite(-80).loadGraphic('assets/images/vignette.png');
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				case 'school' | 'schoolEvil' | 'computerpixel':
				...
				default:
					babyArrow.frames = FlxAtlasFrames.fromSparrow('assets/images/NOTE_assetsStinky.png', 'assets/images/NOTE_assetsStinky.xml');
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == "jailbreak" && doof.dialogueNum == 10 && gfRemoved == false){
			remove(gf);
			gfRemoved = true;
			//gf.animation.play('SHESFUCKINGGONE');
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			{
				camFocusOpponent();
				FlxTween.tween(FlxG.camera, {zoom: .9}, 1.70, {ease: FlxEase.expoInOut});
				defaultCamZoom = .9;
			}
			...
			{
				camFocusBF();
				FlxTween.tween(FlxG.camera, {zoom: .8}, 1.70, {ease: FlxEase.expoInOut});
				defaultCamZoom = .8;
			}
		}
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0)
			{
				...
				if (storyDifficulty == 2){
					...
					FlxG.switchState(new GameOverBad());
					sectionStart = false;
				}
	...
	override function stepHit()
	{
		...
		if (curSong.toLowerCase() == 'sensory-overload'){
			if (curStep == 958)
				{
			...
			if (curStep == 925)
				{
					dad.playAnim('specialAnimYeah', true);
				}
			if (curStep == 927)
			{
			...
			switch (curStep){
				case 824| 825 | 826 | 827:
					if (hasChanged == false){
						error.alpha = 1;
						camHUD.zoom += 0.03;
						hasChanged = true;
					}
				case 830 | 831 |832 |833|834|835:
					if (errorRemoved == false){
						error.alpha = 0;
						remove(error);
						errorRemoved = true;
					}		
		...
		if (curSong.toLowerCase() == 'torrent')
			{
				if (curStep == 111){
					dad.playAnim('specialAnim', true);
	...
	override function beatHit()
	{
		...
		if (curBeat % 1 == 0){
			...
			FlxTween.tween(iconP1, {iconScale: iconP1.defualtIconScale}, 0.3, {ease: FlxEase.quintOut});
			FlxTween.tween(iconP2, {iconScale: iconP2.defualtIconScale}, 0.3, {ease: FlxEase.quintOut});
		}
		...
		switch (curStage)
		{
			...
			case "computer":
				...
				if (totalBeats % 4 == 0)
				{
					torrentBack.forEach(function(monitorColor:FlxSprite)
					{
						monitorColor.visible = false;
					});

					curMonitor = FlxG.random.int(0, torrentBack.length - 1);

					torrentBack.members[curMonitor].visible = true;
					// phillyCityLights.members[curLight].alpha = 1;
				}
				nonbiney.animation.play('binaryglowin', false);
			case "computer-evil":
	...
	function camFocusOpponent(){
		...
		if (SONG.song.toLowerCase() == 'firewall' || SONG.song.toLowerCase() == 'bitcrush')
		{
			camTween = FlxTween.tween(camFollow, {x: followX, y: followY}, 1.45, {ease: FlxEase.quartInOut});
		}
		else
			camTween = FlxTween.tween(camFollow, {x: followX, y: followY}, 1.70, {ease: FlxEase.quartInOut});
	}
