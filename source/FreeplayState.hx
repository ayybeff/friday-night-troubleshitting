...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (StoryMenuState.weekUnlocked[2] || isDebug)
			{
				addWeek(['torrent', 'sensory-overload', 'jailbreak', 'bitcrush', 'firewall', 'subzero'], 1, ['torrent']);
			}
		// LOAD MUSIC
		/*for(x in songs){
		...
		{
			var songText:Alphabet = new Alphabet(0, (100 * i) + 100, songs[i].songName, true, false);
				...
				switch (songs[curSelected].songName.toLowerCase()){
					...
					case 'subzero':
						if (curDifficulty == 2){
							...
							FlxG.sound.play('assets/sounds/noNo' + TitleState.soundExt);
							curDifficulty = 2;
							changeDiff();
						}
						default:
